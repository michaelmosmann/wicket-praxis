	...
	public void respond(RequestCycle requestCycle) {
		...
		if (_redirectUrl.startsWith("/")) {
			...
			if (rc.isPortletRequest() && ((PortletRequestContext) rc).isEmbedded()) {
				...
				String location = RequestCycle.get().getProcessor().getRequestCodingStrategy().rewriteStaticRelativeUrl(
						_redirectUrl.substring(1));
			...
			// relative url
			redirect(response, RequestCycle.get().getRequest().getRelativePathPrefixToWicketHandler() + _redirectUrl);
		}
	}
